...
`network_upstream_timeout_total` is a global network metric and does not carry `outbound_profile`. Its `stage` label identifies where the shared query deadline expired. Current values are `pool_acquire`, `connection_create`, `protocol_handshake`, and `query_io`, which distinguish pool waiting, connection establishment, protocol negotiation, and query I/O timeouts.
