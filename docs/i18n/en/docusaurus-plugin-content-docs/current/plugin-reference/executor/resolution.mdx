...
- Type: `boolean`; Required: no; Default: `false`
- Purpose: Select HTTP POST for DoH/DoH3 requests.
- Behavior:
  - `false`: uses RFC 8484 GET and carries the DNS wire message in the URL `dns=` parameter.
  - `true`: uses POST with the raw `application/dns-message` wire message in the request body.
  - Fixed query parameters configured in the DoH URL are preserved for both GET and POST.
  - Non-DoH upstreams such as UDP, TCP, DoT, and DoQ ignore this field.
...
- tag: cache_main
  ...
  args:
    ...
    lazy_cache_ttl: 120
    # Bound concurrent background lazy refreshes across distinct cache keys
    lazy_refresh_concurrency: 64
    # Maximum wall-clock time for one background lazy refresh, in seconds
    lazy_refresh_timeout: 10
    # Minimum retry delay after a failed lazy refresh, in seconds
...
- Type: `integer`; Required: no; Default: `64`
- Purpose: Bound concurrent lazy refreshes across distinct cache keys.
- Behavior:
  - The existing per-key deduplication still prevents duplicate refreshes for the same key.
  - When all refresh slots are busy, stale responses are still served, but no task is queued or spawned.
  - Refreshes skipped by the concurrency limit increment `cache_lazy_refresh_total{result="skipped_busy"}`.
  - A later stale request can retry once a refresh slot becomes available.

#### `lazy_refresh_timeout`

- Type: `integer`; Required: no; Default: `10`
- Unit: seconds
- Purpose: Limit the wall-clock time for one background lazy refresh.
...
- Unit: seconds
- Purpose: Bound retry frequency for the same stale cache key after a lazy refresh failure.
- Behavior:
  - Upstream errors, timeouts, SERVFAIL/unusable responses, and similar refresh failures arm a cooldown on the current stale entry.
  - During the cooldown, stale responses are still served but no new background refresh is started for that entry.
  - Refreshes skipped by the failure cooldown increment `cache_lazy_refresh_total{result="skipped_cooldown"}`.
  - A successful refresh publishes a new cache entry, so the previous entry's failure cooldown is not inherited.
  - The 30-second default follows the RFC 8767 failure-recheck recommendation.
...
- Purpose: Minimum interval between automatic periodic dump checks.
- Behavior:
  - Automatic periodic dumps never run more frequently than this configured interval.
  - The internal dirty-age policy may defer a low-churn dump, but never shortens `dump_interval`.
  - The final dump during shutdown and explicit API-triggered dumps are not delayed by this interval.
...
- Type: `boolean`; Required: no; Default: `false`
- Purpose: Control whether ECS participates in cache keying.
- `false`: ECS from the client request is still forwarded unchanged to downstream/upstream plugins, but the cache key ignores ECS. ECS is stripped from the upstream response before the cache stage returns or stores it, so clients with different ECS values (or no ECS) share the same ordinary cache entry. This is a first-writer-wins policy: the first successfully cached ECS-steered answer may be reused by other ECS clients.
- `true`: ECS participates in scope-aware cache keying and lookup. Cache isolation and reuse follow RFC 7871 FAMILY, SOURCE PREFIX-LENGTH, ADDRESS, and response SCOPE PREFIX-LENGTH semantics.
- Regardless of this setting, invalid request ECS (for example, a non-zero SCOPE PREFIX-LENGTH or an out-of-range prefix) bypasses caching and continues through the downstream chain.
