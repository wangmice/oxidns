...
Upgrades automatically use the new storage while keeping configuration and existing APIs compatible. **History starts afresh; old history is no longer displayed.** Old data is not automatically deleted and continues to occupy space. Neither “clear history” nor retention cleanup removes it. To reclaim that space, back up first and remove only the intended recorder's old tables; never delete a potentially shared database file.
