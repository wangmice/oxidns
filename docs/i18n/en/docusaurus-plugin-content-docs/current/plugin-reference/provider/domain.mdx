...
- Runtime impact:
  ...
  - When `runtime.provider_file_auto_reload` is enabled, external changes to these files automatically trigger a provider reload; bursts are coalesced with an approximately `500ms` trailing debounce.
...
- Supports independent refresh through `reload_provider` or `POST /plugins/<tag>/reload`; when `runtime.provider_file_auto_reload` is enabled, external changes to `file` also trigger an automatic provider reload.
...
- Initialization and reload stream rule files; when `runtime.provider_file_auto_reload` is enabled, external changes to entries in `files` automatically trigger a provider reload; skipped rules are summarized by reason with at most five samples retained for each reason
