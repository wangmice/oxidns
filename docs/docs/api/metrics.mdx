...
`network_upstream_timeout_total` 是全局网络指标，不带 `outbound_profile`。其 `stage` 标签用于标识上游网络操作发生 deadline 超时的阶段，当前取值为 `pool_acquire`、`connection_create`、`protocol_handshake` 和 `query_io`。其中 `pool_acquire` 和 `query_io` 仅统计前台查询 deadline；`connection_create` 和 `protocol_handshake` 还包含后台连接池扩容的建连与握手超时。该指标用于判断超时主要发生在连接池等待、连接创建、协议握手还是实际查询 I/O 阶段。
