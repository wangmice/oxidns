...
- 类型：`boolean`；必填：否；默认值：`false`
- 作用：控制 DoH/DoH3 是否使用 HTTP POST。
- 行为：
  - `false`：使用 RFC 8484 GET，DNS wire message 通过 URL 的 `dns=` 参数传输。
  - `true`：使用 POST，请求体直接携带 `application/dns-message` wire message。
  - 配置在 DoH URL 中的固定 query 参数在 GET 和 POST 下都会保留。
  - UDP、TCP、DoT、DoQ 等非 DoH upstream 会忽略该字段。
...
- tag: cache_main
  ...
  args:
    ...
    # 单次后台 lazy refresh 的最长 wall-clock 时间（秒）
...
- 类型：`integer`；必填：否；默认值：`10`
- 单位：秒
- 作用：限制单次后台 lazy refresh 的最长 wall-clock 时间。
- 设计说明：默认值刻意长于上游默认 `5s` 查询超时，使 upstream/forward 能先返回自身超时或错误，而不是被 cache 外层 timer 抢先取消。
